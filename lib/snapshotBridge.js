...
async function pushHareketSignals(pool, breakouts) {
  ...
  await runCollector(candidates, 'hareket-bridge');
}

// ESM collector'ı dinamik import (CJS'ten çalışır). Pipeline kendi DB verilerini
// (OI, RSI, hacim, fuel) çeker ve her candidate için tam snapshot + score yazar.
// candidates: [{ symbol(binance_symbol), source_type, price_change, funding_fee, note }]
async function runCollector(candidates, tag = 'snapshot-bridge') {
  if (!candidates || !candidates.length) return;
  try {
    const mod = await import(COLLECTOR_PATH);
    await mod.runSignalSnapshotCollector(candidates);
    console.log(`[${tag}] ${candidates.length} snapshot pipeline'a gönderildi`);
  } catch (e) {
    console.error(`[${tag}] runSignalSnapshotCollector hatası:`, e.message || e);
  }
}

// base_asset → binance_symbol map'i dışarıya da aç (techNotify kullanır)
module.exports = { pushHareketSignals, runCollector, loadFuturesSymbolMap };
