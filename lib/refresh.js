...
async function startAll(pool, onComplete) {
  ...
  if (symbols.length) {
    ...
    // NOT: coin_tech_signals BURADA prune EDİLMEZ — onun evreni daha geniş
    // (tüm TRADING futures) ve kendi prune'unu lib/techScan.js yapar.
    ...
    if (d1.affectedRows || d2.affectedRows)
      console.log(`[pump/refresh] prune: coin_metrics -${d1.affectedRows}, pump_events -${d2.affectedRows}`);
  }
