...
const { techSignals, stochRsi } = require('./tech');
...
const TECH_COLS = ['symbol','timeframe','rsi14','rsi_ma','rsi_cross_bars_ago',
                   ...
                   'stoch_k','stoch_d','stoch_cross_bars_ago','updated_at'];
...
async function scanTechCoin(pool, symbol) {
  ...
  const now = new Date();
  const values = tfRows.map(({ tf, rs, tech, stoch, maSource }) => [
    ...
    stoch.stoch_k, stoch.stoch_d, stoch.stoch_cross_bars_ago, now,
  ]);
