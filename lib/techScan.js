...
const { techSignals, stochRsi, superTrend } = require('./tech');
...
const TECH_COLS = ['symbol','timeframe','rsi14','rsi_ma','rsi_cross_bars_ago',
                   ...
                   'macd','macd_signal','macd_hist','macd_cross_bars_ago',
                   'stoch_k','stoch_d','stoch_cross_bars_ago',
                   'st','st_dir','st_bars','st_dist_pct','updated_at'];
...
async function scanTechCoin(pool, symbol) {
  ...
  for (const [tf, c] of Object.entries(cfg.TIMEFRAMES)) {
    try {
      ...
      // SuperTrend ATR gerektirir → high/low da lazım. parseRows'un wick-clamp'lenmiş
      // low'u kullanılır (bozuk listeleme print'leri ATR'yi şişirmesin).
      const hi = rows.map(r => r.high), lo = rows.map(r => r.low);
      tfRows.push({ tf, rs: rsiSignal(cl), tech: techSignals(cl), stoch: stochRsi(cl),
  ...
  const now = new Date();
  const values = tfRows.map(({ tf, rs, tech, stoch, st, maSource }) => [
    symbol, tf, rs.rsi14, rs.rsi_ma, rs.rsi_cross_days_ago,
    tech.ma50, tech.ma200, tech.ma_cross_days_ago, maSource || tf,
    tech.macd, tech.macd_signal, tech.macd_hist, tech.macd_cross_days_ago,
    stoch.stoch_k, stoch.stoch_d, stoch.stoch_cross_bars_ago,
    st.st, st.st_dir, st.st_bars, st.st_dist_pct, now,
  ]);
...
// Arka planda başlat (endpoint için) — hemen döner, ilerleme status() ile okunur.
// Scheduler runAll'ı await ederek kullanır (techNotify'ın öncesinde bitmesi gerekir).
function start(pool) {
  if (job.running) return { started: false, reason: 'already_running' };
  runAll(pool).catch(e => console.error('[tech-scan] hata:', e.message));
  return { started: true };
}

module.exports = { getTechUniverse, scanTechCoin, runAll, start, status };
