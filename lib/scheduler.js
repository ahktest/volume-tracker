...
const { pushHareketSignals } = require('./snapshotBridge');
const techNotify = require('./techNotify');
const techScan = require('./techScan');
...
async function breakoutCheck(pool) {
  ...
  const skippedPrev = [], skippedCooldown = [];
  const fresh = result.breakouts.filter(b => {
    if (firingPrev.has(b.symbol)) { skippedPrev.push(b.symbol); return false; }
    if ((now - (lastAlertAt.get(b.symbol) || 0)) < COOLDOWN_MS) { skippedCooldown.push(b.symbol); return false; }
    return true;
  });
  firingPrev = firingNow;

  // her turda durum logu (neden mesaj atıldı/atılmadı görünsün)
  console.log(`[scheduler] breakout kontrol: ${firingNow.size} yanıyor [${[...firingNow].join(', ') || '-'}]`
    + ` | yeni: ${fresh.length}`
    + (skippedPrev.length ? ` | zaten-yanıyordu: ${skippedPrev.join(', ')}` : '')
    + (skippedCooldown.length ? ` | cooldown: ${skippedCooldown.join(', ')}` : ''));
  ...
  // Fresh breakout'ları signal_snapshots pipeline'ına düşür (source_type='hareket',
  // sadece kayıt). Hata scheduler'ı düşürmez — bridge kendi içinde try/catch'li.
  try {
    await pushHareketSignals(pool, fresh);
  } catch (e) {
    console.error('[scheduler] hareket bridge hatası:', e.message);
  }
}

// Tam refresh (alpha∩futures) → teknik tarama (tüm futures) → bildirim. Günde 4×.
function refreshAndNotify(pool, tag) {
  console.log(`[scheduler] ${tag} tam refresh başlıyor`);
  refresh.startAll(pool, async () => {
    try { await techScan.runAll(pool); }
    catch (e) { console.error('[scheduler] techScan hatası:', e.message); }
    try { await techNotify.run(pool); }
    catch (e) { console.error('[scheduler] techNotify hatası:', e.message); }
  }).then(r => {
    if (!r.started) console.warn(`[scheduler] ${tag} refresh başlamadı: ${r.reason} (bir önceki hâlâ çalışıyor)`);
  }).catch(e => console.error(`[scheduler] ${tag} refresh hatası:`, e.message));
}

function start(pool) {
  // Günde 4× tam refresh (00/07/13/19 UTC+3) → her biri bitince teknik takip bildirimi
  cron.schedule(cfg.CRON_REFRESH, () => refreshAndNotify(pool, 'cron'), { timezone: cfg.CRON_TZ });

  // 15dk breakout kontrol + telegram (hareket başladı — ayrı grup)
  ...
  console.log(`[scheduler] başlatıldı — refresh ${cfg.CRON_REFRESH} (${cfg.CRON_TZ}) + teknik takip bildirimi + ${cfg.CRON_BREAKOUT} breakout`);
}
