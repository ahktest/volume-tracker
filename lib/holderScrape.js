...
// Cloudflare sadece User-Agent'a değil başlık setinin BÜTÜNÜNE bakıyor: gerçek Chrome'un
// gönderdiği Sec-Fetch-*/sec-ch-ua/Accept-Language eksikse istek "otomasyon" olarak kokuyor.
// NOT: Bu, IP itibarına dayalı engeli ÇÖZMEZ — datacenter IP'si engelliyse başlık fark etmez.
...
async function fetchAndParse(tok) {
  ...
  let html = null, lastErr = null, lastStatus = null, lastSnippet = '';
  for (let i = 0; i <= cfg.HOLDERS_RETRIES; i++) {
    try {
      ...
      lastSnippet = String(res.data || '').replace(/\s+/g, ' ').slice(0, 200);
      lastErr = new Error(`HTTP ${res.status}`);
      console.error(`[holders] ${tok.symbol} deneme ${i + 1}/${cfg.HOLDERS_RETRIES + 1} — ` +
        `${tok.explorer} HTTP ${res.status} | ${url}\n         gövde: ${lastSnippet}`);
    } catch (e) {
      lastErr = e;
      console.error(`[holders] ${tok.symbol} deneme ${i + 1}/${cfg.HOLDERS_RETRIES + 1} — ` +
        `${tok.explorer} istek hatası: ${e.message}`);
    }
    if (i < cfg.HOLDERS_RETRIES) await new Promise(r => setTimeout(r, 800 * (i + 1)));
  }

  if (!html) {
    // 403 iki farklı şey olabilir ve ayrımı önemli:
    //  · gövde Cloudflare challenge'ı ise → bot doğrulaması (tarayıcıda açılır, sunucudan açılmaz)
    //  · gövde başka bir şeyse → büyük olasılıkla IP itibarı (datacenter/VPS IP'si engelli)
    const cf = /just a moment|cf-challenge|attention required|cloudflare|cf-ray/i.test(lastSnippet);
    const why = lastStatus === 403
      ? (cf ? 'Cloudflare bot doğrulaması istedi' : 'sunucu IP\'si engellenmiş olabilir (VPS/datacenter IP itibarı)')
      : (lastErr ? lastErr.message : 'bilinmeyen');
    const e = new Error(`${tok.explorer} erişilemedi (HTTP ${lastStatus ?? '—'}): ${why}`);
    e.status = 502; e.httpStatus = lastStatus; e.cloudflare = cf;
    throw e;
  }
  if (/just a moment|cf-challenge|attention required/i.test(html))
    throw new Error(`${tok.explorer} Cloudflare doğrulaması istedi (HTTP 200 içinde challenge)`);
