...
// opts.force: tazeleme aralığı + hata cooldown'ı atlanır (toplu backfill scripti için;
// o kendi sürecinde sırayla çalıştığı için sunucunun frenlerine ihtiyaç duymaz).
async function refreshOne(pool, symbol, opts = {}) {
  ...
  try {
    ...
    // ── Çekirdek: Chainbase KEŞİF, zincir GERÇEK ──
    ...
    if (chk.verified) {
      console.log(`[holders] ${symbol} zincir doğrulaması: ${rawRows.length} aday → ` +
        `${rows.length} geçerli (düzeltilen ${chk.corrected}, sıfırlanmış ${chk.dropped}, okunamayan ${chk.unknown})` +
        (bridged ? ` · arz: zincir=${Math.round(chk.chainSupply).toLocaleString('tr-TR')} vs Binance=${Math.round(binanceSupply).toLocaleString('tr-TR')} → zincir kullanıldı` : ''));
    } else {
      console.warn(`[holders] ${symbol} zincir doğrulanamadı (${chk.reason}) — Chainbase verisi korundu`);
    }
    ...
    if (bridged) risk.flags.push('kopru_arz');   // zincirdeki arz Binance'in global arzından farklı
    ...
    // NOT: bu sayı bakiyesi sıfırlanmış cüzdanları da içeriyor (aynı bayatlık; ölçümde
    // Chainbase 55.223 derken explorer 16.255 diyordu) → şişkin okunmalı.
