...
async function rpcBatch(rpc, calls) {
  ...
  for (let i = 0; i < calls.length; i += cfg.RPC_BATCH_SIZE) {
    ...
    for (const r of (Array.isArray(res.data) ? res.data : [])) {
      if (r && typeof r.id === 'number' && typeof r.result === 'string') out[r.id] = r.result;
    }
  }
  return out;
}
...
async function verifyOnChain(tok, rows) {
  ...
  try {
    const [decHex] = await rpcBatch(rpc, [{ to: tok.contract, data: '0x313ce567' }]);
    ...
    return { rows: fixed, verified: true, decimals: dec, corrected, dropped, unknown };
  } catch (e) {
