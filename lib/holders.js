...
function computeCohorts(holders, totalSupply) {
  const pctOf = amt => totalSupply > 0 ? (amt / totalSupply) * 100 : 0;
  const out = [];
  for (const [name, from, to, color] of COHORTS) {
    const slice = holders.slice(from, to);
    if (!slice.length) continue;
    const amt = slice.reduce((s, h) => s + h.amount, 0);
    const usd = slice.reduce((s, h) => s + h.usd, 0);
    out.push({ name, value: pctOf(amt), color, amountUsd: usd, amountLabel: fmtUsd(usd) });
  }
  // "Outside 100": toplam arzın ilk 100'de olmayan kısmı. Arz verisi tutarsızsa negatife
  // düşebilir (ör. yakılmış arz totalSupply'dan düşülmemişse) → 0'a kırp.
  const top100Amt = holders.slice(0, 100).reduce((s, h) => s + h.amount, 0);
  const rest = Math.max(0, 100 - pctOf(top100Amt));
  ...
  try {
    ...
    const holders = rows.map((r, i) => {
      ...
      return {
        ...
        quantity: r.amount, usd: Number(r.usd_value) || 0,
        pct: (amount / tok.totalSupply) * 100,
      };
    });

    const cohorts = computeCohorts(holders, tok.totalSupply);
    ...
    const over1k = holders.filter(h => h.usd >= 1000).length;
    const capped = over1k >= holders.length;

    const risk = computeRisk(clean5, holders, capped ? null : over1k, tok);
    if (unlabeled.length < 5) risk.flags.push('temiz_top5_eksik');
    if (!labelMap.size && !poolSet.size) risk.flags.push('etiket_yok');
...
module.exports = {
  ...
  // tanı scripti için
  fetchTopHolders, fetchPoolAddresses, fetchLabels, labelKind, computeCohorts,
};
