...
// 'teknik_takip_ps' de sayılır: potansiyel-short bildirimi gönderilmiş kayıtlar
// (teknik-takip-notifier.js source_type'ı böyle güncelliyor) yoksa dedup delinir.
async function dedupInfo(pool, binanceSymbol) {
  const [[today]] = await pool.query(
    `SELECT COUNT(*) AS n FROM signal_snapshots
      WHERE symbol = ? AND source_type IN ('teknik_takip','teknik_takip_ps')
