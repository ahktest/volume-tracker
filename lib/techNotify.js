...
// Kategori: golden = ma50>ma200 & kesişim ≤ MA_CROSS_RECENT_DAYS ; macdPos = macd>0
// RSI (rsi_cross_days_ago ≤ RSI_CROSS_MAX_DAYS) zaten aday filtresinde garanti.
function categorize(c) {
  const f = N(c.ma50), s = N(c.ma200), md = N(c.ma_cross_days_ago);
  const golden = f !== null && s !== null && f > s && md !== null && md <= cfg.MA_CROSS_RECENT_DAYS;
...
function buildMessage(toSend) {
  const sorted = [...toSend].sort((a, b) => ORDER.indexOf(a.cat) - ORDER.indexOf(b.cat)
    || (N(a.c.rsi_cross_days_ago) - N(b.c.rsi_cross_days_ago)));
...
async function run(pool) {
  // 1) Adaylar — RSI kesişi son ≤ RSI_CROSS_MAX_DAYS gün içinde
  const [rows] = await pool.query(
    `SELECT symbol, rsi14, rsi_cross_days_ago, ma50, ma200, ma_cross_days_ago,
            macd, mcap_usd, dist_lo7, last_price
       FROM coin_metrics
      WHERE rsi_cross_days_ago IS NOT NULL AND rsi_cross_days_ago <= ?`,
    [cfg.RSI_CROSS_MAX_DAYS]
  );
