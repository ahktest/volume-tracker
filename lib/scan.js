...
const { computeSqueeze } = require('./squeeze');
...
async function getUniverse(pool) {
  ...
  // binance_futures_tracking meta'sı — YALNIZCA status='TRADING' & delist değil
  // (SETTLING/PENDING_TRADING gibi delisting semboller evren dışı)
  ...
  // TRADING futures listingi olmayan coinleri evrenden çıkar
  const universe = rows.filter(r => bft.has(r.symbol)).map(r => {
  ...
  console.log(`[pump] evren: alpha∩fut=${rows.length}, TRADING=${universe.length} (dışlanan ${rows.length - universe.length})`);
  return universe;
}
...
async function scanCoin(pool, coin) {
  ...
  const sq       = computeSqueeze(primary);   // hacim kuruması + BBW sıkışması (futures serisi)
  const closes   = primary.map(r => r.close);
  const rs       = rsiSignal(closes);   // RSI(14) + MA + aşırı satım kesiş sinyali (günlük)
  const tech     = techSignals(closes); // MA50/200 golden-death + MACD(12/26/9) (günlük)
  ...
  const metrics = {
    ...
    // squeeze metrikleri
    vol_dryup_ratio: sq.vol_dryup_ratio,
    vol_dryup_days:  sq.vol_dryup_days,
    bbw_percentile:  sq.bbw_percentile,
    band_top_20d:    sq.band_top_20d,
    vol_avg_20d:     sq.vol_avg_20d,
    squeeze_metrics_at: new Date(),

    // RSI teknik takip metrikleri
    rsi14:              rs.rsi14,
    rsi_ma:             rs.rsi_ma,
    rsi_cross_days_ago: rs.rsi_cross_days_ago,
    rsi_signal_at:      new Date(),

    // MA (golden/death) + MACD metrikleri
    ma50:                tech.ma50,
    ma200:               tech.ma200,
    ma_cross_days_ago:   tech.ma_cross_days_ago,
    macd:                tech.macd,
    macd_signal:         tech.macd_signal,
    macd_hist:           tech.macd_hist,
    macd_cross_days_ago: tech.macd_cross_days_ago,
  };
  ...
  // Çoklu zaman dilimi teknik sinyalleri (1d/4h/1w) → coin_tech_signals
  // 1d zaten yukarıda hesaplandı, tekrar fetch/hesap yok (drift olmasın).
  const tfRows = [{ tf: '1d', rs, tech }];
  for (const [tf, c] of Object.entries(cfg.TIMEFRAMES)) {
    ...
    } catch (e) { /* bu zaman dilimi yoksa atla */ }
  }
  await upsertTechSignals(pool, symbol, tfRows);
...
const METRIC_COLS = [
  ...
  'vol_dryup_ratio','vol_dryup_days','bbw_percentile','band_top_20d','vol_avg_20d','squeeze_metrics_at',
  'rsi14','rsi_ma','rsi_cross_days_ago','rsi_signal_at',
...
// coin_tech_signals: (symbol, timeframe) başına tek satır. *_bars_ago alanları BAR cinsinden
// (1d→gün, 4h→4 saat, 1w→hafta). Eşik kıyası cfg.TIMEFRAMES[tf] ile yapılır.
const TECH_COLS = ['symbol','timeframe','rsi14','rsi_ma','rsi_cross_bars_ago',
                   'ma50','ma200','ma_cross_bars_ago',
                   'macd','macd_signal','macd_hist','macd_cross_bars_ago','updated_at'];

async function upsertTechSignals(pool, symbol, tfRows) {
  if (!tfRows.length) return;
  const now = new Date();
  const values = tfRows.map(({ tf, rs, tech }) => [
    symbol, tf, rs.rsi14, rs.rsi_ma, rs.rsi_cross_days_ago,
    tech.ma50, tech.ma200, tech.ma_cross_days_ago,
