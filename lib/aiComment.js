...
function holderBlock(h) {
  ...
  const tiers = parse(h.tiers_json) || [];
  const depth = parse(h.depth_json) || [];
  ...
  return {
    ...
    toplam_holder: n(h.holders_total),
    bin_dolar_ustu_cuzdan: over1k && over1k.custom ? n(over1k.custom.rawCount) : null,
    tier_dagilimi: tiers.map(t => ({
      ad: t.name,
      cuzdan: t.custom ? n(t.custom.holders) : null,
      arz_payi_yuzde: t.custom ? n(t.custom.marketCapShare) : null,
    })),
...
- "toplam_holder" ve "bin_dolar_ustu_cuzdan" birlikte gerçek dağılımı gösterir: holder sayısı yüksek ama $1.000 üstü cüzdan çok azsa bunu not et.
