...
async function buildContext(pool, symbol) {
  ...
  const [tech] = await pool.query(
    `SELECT timeframe, rsi14, rsi_ma, rsi_cross_bars_ago, ma50, ma200, ma_cross_bars_ago,
            ma_source, macd, macd_signal, macd_hist, stoch_k, stoch_d, stoch_cross_bars_ago
  ...
  // Holder dağılımı — coin_holders'ta satır varsa. Satır YOKSA context'e alan hiç eklenmez;
  // prompt'un koşullu dalı bunu görüp eski davranışa (on-chain veri yok) düşer.
  let h = null;
  try { [[h]] = await pool.query('SELECT * FROM coin_holders WHERE symbol = ?', [symbol]); }
  ...
  return {
    ...
    teknik_zaman_dilimleri: tech.map(t => ({
      ...
      stoch_k: num(t.stoch_k), stoch_d: num(t.stoch_d),   // StochRSI %K/%D (0-100; <10 aşırı satım, >90 aşırı alım)
      stoch_yukari_kesis_kac_bar_once: num(t.stoch_cross_bars_ago),
    })),
    ...
    ...(h ? { holder_dagilimi: holderBlock(h) } : {}),
    veri_tarihi: meta.last_updated_at || null,
  };
}

// coin_holders satırını prompt'un anlayacağı sade bloğa çevirir.
// Kaynak Chainbase: ilk 100 cüzdan. Tüm yüzdelerin paydası TOPLAM ARZ.
function holderBlock(h) {
  const n = v => (v === null || v === undefined) ? null : Number(v);
  const parse = s => { try { return typeof s === 'string' ? JSON.parse(s) : s; } catch { return null; } };
  const top = (parse(h.top_holders_json) || []).slice(0, 10);

  const chains = parse(h.chains_json) || [];

  return {
    zincir: h.chain,
    kaynak: h.source,
    // Token birden fazla ağda olabilir; aşağıdaki oranlar YALNIZ "zincir" alanındaki ağın
    // arzına göredir. Bu ağın payı düşükse oranlar tokenin küçük bir dilimini anlatır.
    bu_zincirin_arz_payi_yuzde: h.supply_pct == null ? null : Number(h.supply_pct),
    aglara_gore_arz_dagilimi: chains.map(c => ({
      ag: c.chain,
      arz_payi_yuzde: c.supplyPct == null ? null : Math.round(Number(c.supplyPct) * 10) / 10,
    })),
    yuzdelerin_paydasi: 'bu zincirdeki kontratın toplam arzı',
    ilk5_yuzde: n(h.top5_pct),
    ilk10_yuzde: n(h.top10_pct),
    ilk25_yuzde: n(h.top25_pct),
    ilk100_yuzde: n(h.top100_pct),
    // ASIL ÖLÇÜ: borsa/havuz/proje adresleri düşülmüş, bağımsız en büyük 5 cüzdanın payı
    temiz_ilk5_yuzde: n(h.clean_top5_pct),
    etiketli_pay_yuzde: n(h.cex_pool_pct),   // CEX + LP havuzu + burn + kurumsal
    // ilk 100 içinde >=$1.000 tutan cüzdan; tavan=true ise gerçek sayı daha yüksek olabilir
    bin_dolar_ustu_cuzdan: n(h.wallets_over_1k),
    bin_dolar_sayimi_tavanda: !!+h.holders_capped,
    dolasim_toplam_arz_orani: n(h.circ_ratio),
    en_buyuk_10: top.map(x => ({
      sira: x.rank, pay_yuzde: n(x.pct),
      etiket: x.label || null,
      tur: x.kind || 'etiketsiz',   // cex | pool | burn | other | etiketsiz
    })),
    konsantrasyon_riski: h.risk_level,      // dusuk | orta | yuksek | cok_yuksek
    risk_bayraklari: h.risk_flags ? String(h.risk_flags).split(',') : [],
    veri_tarihi: h.fetched_at || null,
  };
}
...
TOPLAM 300-400 KELİMEYİ AŞMA. Uzun yazmak kötüdür; seçici ol.

Önce WebSearch/WebFetch ile araştır (1-2 arama yeterli): proje ne yapıyor ve son haberler.
On-chain/holder verisi için ARAMA YAPMA — o veri aşağıda anlatıldığı gibi sana hazır veriliyor.
...
## On-Zincir
Veride "holder_dagilimi" alanı VARSA: en fazla 3 madde, her biri TEK cümle, SADECE o alandaki rakamlardan yaz.
- "temiz_ilk5_yuzde" asıl ölçüdür: borsa cüzdanları, likidite havuzları ve proje/vesting adresleri düşülmüş hâliyle en büyük 5 bağımsız cüzdanın arz payı. "ilk5_yuzde" ise bunları da içeren ham orandır — ikisi arasındaki farkı, anlamlıysa belirt.
- "temiz_ilk5_yuzde" %80'in üzerindeyse bunu açıkça yaz: arzın bu kadarı birkaç bağımsız cüzdanda toplanmışsa, düşük likiditeyle birleştiğinde fiyat az sayıda aktör tarafından oynatılmaya açıktır. "risk_bayraklari" içinde "tek_cuzdan_hakim" varsa tek bir cüzdanın baskınlığını da söyle.
- Bunun bir manipülasyon KANITI değil, zemini olduğunu belirt; ham oranın borsa/havuz adreslerini de içerdiğini unutma.
- "bin_dolar_ustu_cuzdan" gerçek katılımı gösterir: bu sayı düşükse (ör. 100'ün altında) coini gerçekten taşıyan çok az cüzdan var demektir. "bin_dolar_sayimi_tavanda" true ise sayım ilk 100'e takılmıştır, "en az bu kadar" diye oku.
- "dolasim_toplam_arz_orani" düşükse (ör. 0.30 altı) arzın büyük kısmı henüz kilitli demektir; yüksek konsantrasyonun bir kısmı bundan kaynaklanabilir, bunu belirt.
- ÖNEMLİ: "bu_zincirin_arz_payi_yuzde" düşükse (ör. %20'nin altı) yukarıdaki tüm oranlar tokenin YALNIZ o ağdaki dilimi hakkındadır — bunu mutlaka söyle, yoksa okuyan "arzın %90'ı 5 cüzdanda" diye yanlış anlar. "aglara_gore_arz_dagilimi" arzın hangi ağda yoğunlaştığını gösterir; ağırlıklı ağ farklıysa onu da belirt.

Veride "holder_dagilimi" alanı YOKSA: tek satır "On-zincir holder verisi çekilmemiş." yaz, başka bir şey ekleme, arama yapma.

Her iki durumda da rakam/oran UYDURMA — sadece verilen alanlardaki sayıları kullan.
...
## Riskler
En fazla 3 madde, her biri TEK cümle. "holder_dagilimi.konsantrasyon_riski" alanı "yuksek" veya "cok_yuksek" ise bunu mutlaka risk maddelerinden biri yap ve "temiz_ilk5_yuzde" rakamını ver.
...
// Bugün yorum yoksa en son yorumu da döndürür — frontend ondan sadece kalıcı
// "Proje" bölümünü gösterir (proje tanımı eskimez; haber/teknik kısmı eskir).
async function today(pool, symbol) {
  ...
  return { comments: rows, latest, limits: await limits(pool, symbol) };
}
